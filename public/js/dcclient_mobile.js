...
BaseArticle.prototype = {
	...
	decoArticle : function(pure) {
		...
		if (article.userid) {
			article.userpic = '<span class="pic"><a class="userpic" href="http://gallog.dcinside.com/'+article.userid+'" target="_blank"><img width="50" height="50" src="http://gom.heyo.me/articles/userpic/'+article.userid+'"/></a></span>'
	...
	layerImageList : function(list, opt) {
		...
		}).attr('src', '/article_thumb.php?id='+opt+'&url='+url.join('::')));
	...
	layerArticle : function(article, force) {
		...
		article.content = article.content.replace(/by gom\.heyo\.me/g, '');;
		article.content = article.content.replace(/IP[ ]+[0-9]+\.[0-9]+\./g, '');;
	...
	render : function(one) {
		...
		if (this.updateQue.length) {
			for (var i=0; i<this.updateQue.length; i++) {
				...
				dock.html(this.layerArticle(this.articles[this.updateQue[i]]));
			}
			this.updateQue = [];
		}
	...
	update : function(url, callback) {
		...
		$.getJSON(url+'?'+Math.random(), function(data) {
			...
			if (data.optional) {
				self.helperPage = data.optional.page;
				self.helperFirst = data.optional.first_article_id;
				self.helperLast = data.optional.last_article_id;
			}
			self.insertFirst = 0;
			self.insertLast = 0;
			...
			if (data.list) {
				for (var i=0; i<data.list.length; i++) {
					...
					var vo = $.extend({}, ArticleVO, data.list[i]);
...
ArticleView.prototype = {
	...
	render : function(recover) {
		...
		this.container.append(
			$('<div class="inner"></div>').html(this.article.content)
		);
...
//			layer.find('.nic').html(article.name);
	...
	update : function(id, callback) {
		...
		$.getJSON('/articles/article/'+id+'?'+Math.random(), function(data) {
			...
			if (!data) {
				alert('서버 연결이 늦습니다. 잠시후 다시 시도해주세요.');
				return;
			}
			var article = data;
...
var Client = {
	...
	preinit : function() {
		...
		$('#add-button').click(function(e) {
			...
			if ($('#check-all-random').length) {
				data.random = $('#check-all-random').attr('checked')?1:0;
			}
			if ($('#check-hint-random').length && !data.random) {
				data.random = $('#check-hint-random').attr('checked')?1:0;
				data.hint = $('#random-hint').val();
				if (data.random) {
					if (data.hint.length > 10) {
			...
			self.adding = true;
			$.ajaxFileUpload({
				url : '/articles/add_article?'+Math.random(),
				data : data,
	...
	loginOK : function() {
		...
		if (!$('#login-password').val() && !$('#login-cookie').val()) {
